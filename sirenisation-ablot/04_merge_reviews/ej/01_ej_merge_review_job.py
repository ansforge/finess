"""Consolidate EJ review workbooks into current confirmed decisions.

This job validates the completed review batch against the proposal frame, publishes
the realised reviewed sample and one confirmed decision per reviewed EJ, then
...
def run(*, write_outputs: bool = True) -> dict[str, object]:
    ...
    review_root = PATHS.ej_review_root
    plan_path = PATHS.stage02_ej_review_plan
    proposal_path = PATHS.stage02_ej_stratified_proposals

    confirmed_output = PATHS.stage04_ej_confirmed
    output_dir = confirmed_output.parent
    reviewed_sample_parquet = output_dir / "ej_reviewed_sample.parquet"
    reviewed_sample_excel = output_dir / "ej_reviewed_sample.xlsx"
    ...
    reviewed_entity_count = reviewed_sample["EJ"].nunique(dropna=True)
    ...
    print("EJ review consolidation")
    print(f"- review workbooks: {len(loaded_files):,}")
    print(f"- reviewed EJ: {reviewed_entity_count:,}")

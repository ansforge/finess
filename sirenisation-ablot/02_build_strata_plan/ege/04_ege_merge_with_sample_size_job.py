"""Apply manually entered EGE sample sizes to the reviewer-assigned strata plan.

This is the final EGE step of Stage 02. It validates the manual sample-size input
against ``n_EGE_pooled``, adds ``sample_size`` to the plan, and publishes the plan
consumed by the review-export stage.
"""
...
def run(
    *, write_outputs: bool = True, display_outputs: bool = True
    ...
    STRATA_PLAN_PATH = PATHS.stage02_ege_plan_with_reviewers
    SAMPLE_SIZE_PATH = PATHS.ege_sample_size_input
    OUTPUT_STRATA_PLAN_PATH = PATHS.stage02_ege_review_plan
    OUTPUT_REVIEWER_STATISTICS_PATH = (
        OUTPUT_STRATA_PLAN_PATH.parent
        / "ege_reviewer_statistics_with_sample_size.xlsx"
    )
    ...
    result = merge_sample_sizes_into_review_plan(
        strata_plan,
        sample_size_input,
        population_column="n_EGE",
        maximum_sample_size_column="n_EGE_pooled",
    )

    final_plan = result.strata_plan
    reviewer_statistics = result.reviewer_statistics

    log("EGE final review plan")
    log(f"- strata: {len(final_plan):,}")
    log(f"- total EGE population represented: {int(final_plan['n_EGE'].sum()):,}")
    log(f"- pooled EGE available: {int(final_plan['n_EGE_pooled'].sum()):,}")
    log(f"- planned EGE reviews: {int(final_plan['sample_size'].sum()):,}")
    ...
    if write_outputs:
        ...
        log("Saved final EGE review-plan outputs:")
